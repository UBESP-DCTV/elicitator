server <- function(input, output, session) {
    ...
    usr  <- reactiveValues(login = FALSE, type = "user")
    ...
    observe({
        ...
        if (!nrow(usr_info)) {
            ...
            observe(
                if (usr$type == "admin") {
                    updateTabItems(session, "tabs", selected = "admin_dashboard")
                } else {
    ...
    output$sidebarpanel <- renderUI({
        ...
        sidebarMenu(
            ...
            # On the dashboard section the numeric imput panel has
            # been also provided if not an admin.
            if (usr$type != "admin") {
                fluidPage(
                    title = "Percentiles",
                    status = "warning", "", br(), "",
    ...
    output$body <- renderUI({
        ...
        if (usr$login && (usr$type == "user")) {
            ...
            dashboardBody(tabItems(tab_page, help_page))
        } else if (!usr$login) {
            login_page
        }
